...
const Level = (props) => {
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelecteds = levelsTable.map((n) => n.id);
    ...
    const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - levelsTable.length) : 0;

    const filteredUsers = applySortFilter(levelsTable, getComparator(order, orderBy), filterName);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            console.log(level);
            await dispatch(addLevel(level));
            setLevelsTable([...levelsTable, level]);
            setLevel({
                level: ''
            });
