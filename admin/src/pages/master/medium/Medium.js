...
const Medium = (props) => {
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelecteds = mediumsTable.map((n) => n.id);
    ...
    const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - mediumsTable.length) : 0;

    const filteredUsers = applySortFilter(mediumsTable, getComparator(order, orderBy), filterName);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            console.log(medium);
            await dispatch(addMedium(medium));
            setMediumsTable([...mediumsTable, medium]);
            setMedium({
                medium: ''
            });
            alert('medium submitted successfully');
        } catch (error) {
            console.log(error);
        }
    };
