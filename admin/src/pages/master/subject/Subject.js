...
const Subject = (props) => {
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelecteds = subjectsTable.map((n) => n.id);
    ...
    const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - subjectsTable.length) : 0;

    const filteredUsers = applySortFilter(subjectsTable, getComparator(order, orderBy), filterName);
